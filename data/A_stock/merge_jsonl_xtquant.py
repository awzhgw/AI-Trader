...
def convert_xt_to_jsonl(
    csv_path: str = "A_stock_data/daily_prices_sse_50_xt.csv",
    output_path: str = "merged.jsonl",
    stock_name_csv: str = "A_stock_data/sse_50_weight.csv",
) -> None:
    """Convert XtQuant A-share CSV data to JSONL format.

    Args:
        csv_path: Path to the XtQuant daily price CSV file
        output_path: Path to output JSONL file
        stock_name_csv: Path to SSE 50 weight CSV containing stock names
    """
    csv_path = Path(__file__).parent / csv_path
    output_path = Path(__file__).parent / output_path
    stock_name_csv = Path(__file__).parent / stock_name_csv
    ...
    with open(output_path, "w", encoding="utf-8") as fout:
        for ts_code, group_df in grouped:
            ...
            for idx, row in group_df.iterrows():
                ...
                # XtQuant volume is typically in shares (股), unlike Tushare which is in hands (手)
                # So we use the volume directly.
                volume = str(int(row["vol"])) if pd.notna(row["vol"]) else "0"
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("A-Share Data Converter (XtQuant)")
    print("=" * 60)
    convert_xt_to_jsonl()
    print("=" * 60)
